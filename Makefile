...
cwd = $(shell pwd)
SHELL = /bin/sh
RUN_APP_ARGS = --rm --user "$(shell id -u):$(shell id -g)" -v"$(cwd)":/app:cached cloud-payments-laravel
...
test-cover: ## Execute php tests with coverage
	$(docker_bin) run --rm --user "0:0" app sh -c 'docker-php-ext-enable xdebug && su $(shell whoami) -s /bin/sh -c "composer phpunit-cover"'
